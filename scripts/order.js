...
(() => {
  if (!window.AxeCart) return;
  const { read, total, remove, clear, itemTotal } = window.AxeCart;
  ...
  form?.addEventListener('submit', (e) => {
    ...
    const location = 'Axe to Grind Café';
    const method = data.get('method') === 'curbside' ? 'Curbside' : 'In-store';
    ...
    const orderId = 'ATG-' + Date.now().toString(36).toUpperCase().slice(-6);
