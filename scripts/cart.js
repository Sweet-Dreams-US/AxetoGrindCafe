// Axe to Grind Café — demo cart with per-item modifiers (localStorage)
(() => {
  const KEY = 'axeCart';

  const MODIFIERS = {
    playlist: [
      { name: 'Hot or Iced', type: 'single', required: true, options: [
      ...
      { name: 'Extras', type: 'multi', options: [
        ...
        { label: 'Decaf', price: 0 },
        { label: 'Whipped cream', price: 0 },
        { label: 'Sugar-free syrup', price: 0 }
      ]}
    ],
    food: [
      { name: 'Add a side', type: 'single', required: true, options: [
        { label: 'No side', price: 0, default: true },
        { label: 'Chips & dill spear', price: 2.00 }
      ]},
      { name: 'Upgrade bread', type: 'single', options: [
        { label: 'Standard bread', price: 0, default: true },
        { label: 'Croissant roll', price: 1.00 }
      ]},
      { name: 'Add protein', type: 'multi', options: [
        { label: 'Bacon', price: 2.00 },
        { label: 'Sausage', price: 2.00 }
      ]}
    ],
    others: [
      { name: 'Flavor', type: 'single', options: [
        { label: 'House pick', price: 0, default: true },
        { label: 'Strawberry', price: 0 },
        { label: 'Raspberry', price: 0 },
        { label: 'Blackberry', price: 0 },
        { label: 'Peach', price: 0 },
        { label: 'Pineapple', price: 0 },
        { label: 'Lime', price: 0 },
        { label: 'Blue cotton candy', price: 0 }
      ]},
      { name: 'Make it sugar-free', type: 'single', options: [
  ...
  window.AxeCart = { read, add, setQty, remove: removeItem, clear, total, count, itemTotal };
  ...
  const renderEmpty = () => el('div', { class: 'cart-empty' },
    ...
    el('p', { class: 'mt-3', style: 'font-size: 0.9rem;' }, 'Add a latte, a waffle, a scone — pick a track.')
  );
